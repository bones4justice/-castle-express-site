...
export function parseInline(text) {
  ...
  return parts.map((part, j) => {
    ...
    if (linkMatch) {
      ...
      const isProtocolLink = /^(tel|mailto|sms|fax):/i.test(href);
      if (isAbsoluteHttp) {
      ...
      if (isProtocolLink) {
        return <a key={j} href={href} style={{ color: "#D4A017", fontWeight: 600, textDecoration: "underline", textUnderlineOffset: 2 }}>{linkMatch[1]}</a>;
      }
